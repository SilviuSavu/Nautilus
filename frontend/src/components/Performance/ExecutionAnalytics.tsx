...
export const ExecutionAnalytics: React.FC<ExecutionAnalyticsProps> = ({ className }) => {
  ...
  const getLatencyTrendIcon = (trend: string) => {
    switch (trend) {
      case 'improving': return <TrendingDownOutlined style={{ color: '#52c41a' }} />;
      case 'degrading': return <TrendingUpOutlined style={{ color: '#ff4d4f' }} />;
    ...
    <div className={`execution-analytics ${className || ''}`}>
      <Card>
        ...
        <Tabs activeKey={activeTab} onChange={setActiveTab}>
          ...
          <TabPane tab="Trade Quality" key="quality">
            {renderTradeQuality()}
          </TabPane>
          
          <TabPane tab="Slippage Analysis" key="slippage">
            {renderSlippageAnalysis()}
          </TabPane>
          
          <TabPane tab="Latency Analysis" key="latency">
