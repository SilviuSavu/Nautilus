...
app = FastAPI(
    title="Nautilus Trader API (Test Mode)", description="Minimal REST API for testing authentication", version="1.0.0-test", lifespan=lifespan
)

# Configure CORS
app.add_middleware(
    CORSMiddleware, allow_origins=settings.cors_origins.split(", "), allow_credentials=True, allow_methods=["*"], allow_headers=["*"], )
...
async def health_check() -> dict[str, Any]:
    ...
    return {
        "status": "healthy", "environment": settings.environment, "mode": "test", "message": "Backend ready for authentication testing"
    }
...
async def get_admin_api_key():
    ...
    return {
        "username": "admin", "user_id": admin_user.id, "api_key": user_dict["api_key"], "created_at": admin_user.created_at.isoformat(), "warning": "This is a debug endpoint - remove in production!"
    }

if __name__ == "__main__":
    import uvicorn
    uvicorn.run(
        "test_main:app", host=settings.host, port=settings.port, reload=settings.debug, log_level="info" if settings.debug else "warning"
    )