...
class HistoricalDataQuery:
    ...
    timeframe: Optional[str] = None  # For bars
...
class HistoricalDataService:
    ...
    def __init__(
        self,
        database_url: str = "postgresql://nautilus@localhost:5432/nautilus",
        pool_min_size: int = 5,
    ...
    async def _create_tables(self, conn: asyncpg.Connection) -> None:
        ...
        if self._timescale_enabled:
            try:
                ...
                """)
                await conn.execute("""
                    SELECT create_hypertable('market_quotes', 'timestamp_ns',
                        chunk_time_interval => 86400000000000, if_not_exists => TRUE);
                """)
                await conn.execute("""
                    SELECT create_hypertable('market_bars', 'timestamp_ns',
            ...
            async with self._pool.acquire() as conn:
                await conn.execute("""
                    INSERT INTO market_ticks (
                        venue, instrument_id, timestamp_ns, price, size, 
                        side, trade_id, sequence_num, raw_data
            ...
            async with self._pool.acquire() as conn:
                ...
                await conn.execute("""
                    INSERT INTO market_quotes (
                        ...
                        bid_size, ask_size, spread, sequence_num, raw_data
            ...
            async with self._pool.acquire() as conn:
                await conn.execute("""
                    INSERT INTO market_bars (
                        venue, instrument_id, timeframe, timestamp_ns,
                        open_price, high_price, low_price, close_price,
                        volume, is_final, raw_data
    ...
    async def query_ticks(self, query: HistoricalDataQuery) -> List[Dict[str, Any]]:
        ...
        try:
            async with self._pool.acquire() as conn:
                sql = """
                    SELECT venue, instrument_id, timestamp_ns, price, size, 
                           side, trade_id, sequence_num, created_at
                    FROM market_ticks
                ...
                params = [
                    query.venue,
                    query.instrument_id,
                    int(query.start_time.timestamp() * 1_000_000_000),
                    int(query.end_time.timestamp() * 1_000_000_000),
                ]
    ...
    async def query_quotes(self, query: HistoricalDataQuery) -> List[Dict[str, Any]]:
        ...
        try:
            async with self._pool.acquire() as conn:
                sql = """
                    SELECT venue, instrument_id, timestamp_ns, bid_price, ask_price,
                           bid_size, ask_size, spread, sequence_num, created_at
                    FROM market_quotes
                    WHERE venue = $1 AND instrument_id = $2
                      AND timestamp_ns >= $3 AND timestamp_ns <= $4
                    ORDER BY timestamp_ns DESC
    ...
    async def query_bars(self, query: HistoricalDataQuery) -> List[Dict[str, Any]]:
        ...
        try:
            async with self._pool.acquire() as conn:
                sql = """
                    SELECT venue, instrument_id, timeframe, timestamp_ns,
                           open_price, high_price, low_price, close_price,
                           volume, is_final, created_at
                    FROM market_bars
                    WHERE venue = $1 AND instrument_id = $2
                      AND timestamp_ns >= $3 AND timestamp_ns <= $4
                """
    ...
    async def get_data_summary(self, venue: str, instrument_id: str) -> Dict[str, Any]:
        ...
        try:
            async with self._pool.acquire() as conn:
                ...
                if tick_row['count'] > 0:
                    summary['ticks'] = {
                        'count': tick_row['count'],
                        'first_timestamp': tick_row['first_timestamp'],
                        'last_timestamp': tick_row['last_timestamp'],
                        'date_range_days': (tick_row['last_timestamp'] - tick_row['first_timestamp']) / (1_000_000_000 * 86400)
                    }
                ...
                if quote_row['count'] > 0:
                    summary['quotes'] = {
                        'count': quote_row['count'],
                        'first_timestamp': quote_row['first_timestamp'],
                        'last_timestamp': quote_row['last_timestamp'],
                        'date_range_days': (quote_row['last_timestamp'] - quote_row['first_timestamp']) / (1_000_000_000 * 86400)
                    }
                ...
                if bar_rows:
                    ...
                    for row in bar_rows:
                        summary['bars'][row['timeframe']] = {
                            ...
                            'date_range_days': (row['last_timestamp'] - row['first_timestamp']) / (1_000_000_000 * 86400)
                        }
            ...
            async with self._pool.acquire() as conn:
                ...
                stats = await conn.fetchrow("""
                    SELECT 
                        ...
                        (SELECT COUNT(*) FROM market_ticks) as tick_count,
                        (SELECT COUNT(*) FROM market_quotes) as quote_count,
                        (SELECT COUNT(*) FROM market_bars) as bar_count
                """)
...
import os
database_url = os.getenv("DATABASE_URL", "postgresql://nautilus@localhost:5432/nautilus")
