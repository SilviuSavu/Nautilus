...
class MarketImpactAnalysis:
    ...
    market_price_before: Decimal | None
    market_price_after: Decimal | None
...
class TradeDataProcessor:
    ...
    async def _calculate_symbol_analytics(self, symbol: str, trades: list[Trade]) -> TradeAnalytics:
        ...
        return TradeAnalytics(
            symbol=symbol, period_start=sorted_trades[0].execution_time, period_end=sorted_trades[-1].execution_time, total_trades=total_trades, total_volume=total_volume, total_pnl=total_pnl, win_rate=win_rate, profit_factor=profit_factor, max_drawdown=max_drawdown, sharpe_ratio=sharpe_ratio, calmar_ratio=calmar_ratio, sortino_ratio=sortino_ratio, average_holding_period=avg_holding_period, max_consecutive_wins=consecutive_wins, max_consecutive_losses=consecutive_losses, largest_win=largest_win, largest_loss=largest_loss, risk_adjusted_return=risk_adjusted_return
        )
    ...
    def _track_positions(self, trades: list[Trade]) -> dict[str, Dict]:
        ...
        for trade in trades:
            ...
            if key not in positions:
                positions[key] = {
                    'quantity': Decimal('0'), 'cost_basis': Decimal('0'), 'realized_pnl': Decimal('0'), 'trades': []
                }
    ...
    def _empty_analytics(self, symbol: str) -> TradeAnalytics:
        ...
        now = datetime.now()
        return TradeAnalytics(
            symbol=symbol, period_start=now, period_end=now, total_trades=0, total_volume=Decimal('0'), total_pnl=Decimal('0'), win_rate=0.0, profit_factor=0.0, max_drawdown=Decimal('0'), sharpe_ratio=None, calmar_ratio=None, sortino_ratio=None, average_holding_period=timedelta(0), max_consecutive_wins=0, max_consecutive_losses=0, largest_win=Decimal('0'), largest_loss=Decimal('0'), risk_adjusted_return=0.0
        )
    ...
    async def _analyze_single_trade_impact(self, trade: Trade) -> MarketImpactAnalysis | None:
        ...
        try:
            ...
            return MarketImpactAnalysis(
                symbol=trade.symbol, trade_id=trade.trade_id, execution_price=trade.price, market_price_before=None, # Would be populated from historical data
    ...
    async def export_to_nautilus_format(self, trades: list[Trade], output_path: str) -> bool:
        ...
        try:
            ...
            for trade in trades:
                execution = {
                    'venue': trade.venue, 'instrument_id': trade.symbol, 'exec_id': trade.execution_id or trade.trade_id, 'order_id': trade.order_id, 'side': trade.side, 'quantity': str(trade.quantity), 'price': str(trade.price), 'commission': str(trade.commission), 'timestamp': int(trade.execution_time.timestamp() * 1_000_000_000), # nanoseconds
                    'account_id': trade.account_id, 'strategy_id': trade.strategy or 'unspecified'
                }
...
_trade_processor: TradeDataProcessor | None = None
