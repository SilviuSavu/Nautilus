...
class ExchangeCredentials:
    ...
    api_key: str
    api_secret: str
    passphrase: str | None = None  # For some exchanges like Coinbase
    sandbox: bool = True  # Default to sandbox/testnet
    base_url: str | None = None  # Custom base URL for testnet
    ws_url: str | None = None    # Custom WebSocket URL for testnet
...
class ExchangeService:
    ...
    def _load_configurations(self) -> None:
        ...
        exchanges = {
            Venue.BINANCE: "BINANCE", Venue.COINBASE: "COINBASE", Venue.KRAKEN: "KRAKEN", Venue.BYBIT: "BYBIT", Venue.OKX: "OKX", }
        ...
        for venue, env_prefix in exchanges.items():
            ...
            if config:
                ...
                self._connections[venue] = ExchangeConnection(
                    venue=venue, status=ExchangeStatus.DISCONNECTED, config=config, supported_features=self._get_supported_features(venue)
                )
                
    def _load_exchange_config(self, venue: Venue, env_prefix: str) -> ExchangeConfig | None:
        ...
        if not api_key or not api_secret:
            ...
            return ExchangeConfig(
                venue=venue, enabled=False, trading_mode=TradingMode.PAPER
            )
        ...
        credentials = ExchangeCredentials(
            api_key=api_key, api_secret=api_secret, passphrase=passphrase, sandbox=sandbox, base_url=base_url, ws_url=ws_url
        )
        
        return ExchangeConfig(
            venue=venue, credentials=credentials, trading_mode=trading_mode, enabled=True, max_orders_per_minute=int(os.getenv(f"{env_prefix}_MAX_ORDERS_PER_MINUTE", "100")), position_size_limit=float(os.getenv(f"{env_prefix}_POSITION_LIMIT", "10000"))
        )
        
    def _get_supported_features(self, venue: Venue) -> list[str]:
        """Get supported features for an exchange"""
        feature_map = {
            Venue.BINANCE: ["spot_trading", "futures_trading", "margin_trading", "websocket_streams"], Venue.COINBASE: ["spot_trading", "websocket_streams", "advanced_orders"], Venue.KRAKEN: ["spot_trading", "futures_trading", "websocket_streams"], Venue.BYBIT: ["spot_trading", "derivatives_trading", "websocket_streams"], Venue.OKX: ["spot_trading", "futures_trading", "options_trading", "websocket_streams"], }
    ...
    def get_exchange_status(self, venue: Venue) -> ExchangeConnection | None:
    ...
    def get_trading_summary(self) -> dict[str, Any]:
        ...
        return {
            "total_exchanges": total_exchanges, "enabled_exchanges": enabled_exchanges, "connected_exchanges": connected_exchanges, "trading_modes": trading_modes, "supported_venues": [venue.value for venue in self._connections.keys()]
        }
