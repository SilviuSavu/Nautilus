...
class DataBackfillService:
    ...
    Features:
    - Graceful disconnect handling: Stops immediately when IB Gateway disconnects
    ...
    - Rate limiting to respect IB Gateway limits (50 requests/minute)
    - Automatic chunking of large time ranges (max 30 days per request)
    
    Disconnect Protection:
    - Monitors IB Gateway connection status continuously
    - Detects "Not connected" and "504" error patterns
    - Immediately stops backfill process on connection loss
    - Marks failed requests with "IB Gateway disconnected" error
    ...
    def __init__(self):
        ...
        self.timeframe_config = {
            "1m": {"ib_size": "1 min", "max_duration_days": 10}, "5m": {"ib_size": "5 mins", "max_duration_days": 60}, "15m": {"ib_size": "15 mins", "max_duration_days": 120}, "30m": {"ib_size": "30 mins", "max_duration_days": 240}, "1h": {"ib_size": "1 hour", "max_duration_days": 365}, "2h": {"ib_size": "2 hours", "max_duration_days": 730}, "4h": {"ib_size": "4 hours", "max_duration_days": 1095}, "1d": {"ib_size": "1 day", "max_duration_days": 3650}, "1w": {"ib_size": "1 week", "max_duration_days": 7300}, "1M": {"ib_size": "1 month", "max_duration_days": 18250}
        }
        
        # Popular instruments to prioritize
        self.priority_instruments = [
            # Major US Stocks
            {"symbol": "AAPL", "sec_type": "STK", "exchange": "SMART", "currency": "USD"}, {"symbol": "GOOGL", "sec_type": "STK", "exchange": "SMART", "currency": "USD"}, {"symbol": "MSFT", "sec_type": "STK", "exchange": "SMART", "currency": "USD"}, {"symbol": "AMZN", "sec_type": "STK", "exchange": "SMART", "currency": "USD"}, {"symbol": "TSLA", "sec_type": "STK", "exchange": "SMART", "currency": "USD"}, {"symbol": "NVDA", "sec_type": "STK", "exchange": "SMART", "currency": "USD"}, {"symbol": "META", "sec_type": "STK", "exchange": "SMART", "currency": "USD"}, {"symbol": "NFLX", "sec_type": "STK", "exchange": "SMART", "currency": "USD"}, # Major Forex Pairs
            {"symbol": "EUR", "sec_type": "CASH", "exchange": "IDEALPRO", "currency": "USD"}, {"symbol": "GBP", "sec_type": "CASH", "exchange": "IDEALPRO", "currency": "USD"}, {"symbol": "JPY", "sec_type": "CASH", "exchange": "IDEALPRO", "currency": "USD"}, {"symbol": "AUD", "sec_type": "CASH", "exchange": "IDEALPRO", "currency": "USD"}, # Major Indices ETFs
            {"symbol": "SPY", "sec_type": "STK", "exchange": "SMART", "currency": "USD"}, {"symbol": "QQQ", "sec_type": "STK", "exchange": "SMART", "currency": "USD"}, {"symbol": "IWM", "sec_type": "STK", "exchange": "SMART", "currency": "USD"}, ]
    ...
    async def analyze_missing_data(self, symbol: str, sec_type: str = "STK", exchange: str = "SMART", currency: str = "USD") -> dict[str, list[tuple[datetime, datetime]]]:
        ...
        try:
            ...
            for timeframe in self.timeframe_config.keys():
                ...
                query = HistoricalDataQuery(
                    venue=venue, instrument_id=instrument_id, data_type="bar", start_time=start_date, end_time=end_date, timeframe=timeframe
                )
    ...
    async def start_backfill_process(self):
        """Start the backfill process with disconnect protection
        
        Features:
        - Verifies IB Gateway connection before starting
        - Monitors connection status before each request
        - Automatically stops on disconnect detection
        ...
        try:
            while self.backfill_queue and self.is_running:
                # Check IB Gateway connection before processing each request
                if not self.ib_client.is_connected():
                    self.logger.error("IB Gateway disconnected - stopping backfill process")
                    self.is_running = False
                    break
    ...
    async def _process_backfill_request(self, request: BackfillRequest):
        ...
        for timeframe in request.timeframes:
            ...
            progress = BackfillProgress(
                request_id=request_id, symbol=request.symbol, timeframe=timeframe, total_periods=0, completed_periods=0, success_count=0, error_count=0, status="running", start_time=datetime.now()
            )
    ...
    async def _backfill_timeframe(self, request: BackfillRequest, timeframe: str, progress: BackfillProgress):
        """Backfill data for a specific timeframe with disconnect detection
        
        Features:
        - Verifies IB Gateway connection before each data request
        - Chunks large time ranges into manageable pieces (max 30 days)
        - Detects connection errors and stops gracefully
        - Implements exponential backoff for temporary errors
        - Stores data in PostgreSQL with proper error handling
        """
        config = self.timeframe_config[timeframe]
        ...
        while current_end > request.start_date and self.is_running:
            current_start = max(request.start_date, current_end - timedelta(days=max_duration_days))
            
            # Check IB Gateway connection before each data request
            if not self.ib_client or not self.ib_client.is_connected():
                self.logger.error(f"IB Gateway disconnected - stopping backfill for {request.symbol} {timeframe}")
                progress.status = "failed"
                progress.last_error = "IB Gateway disconnected"
                self.is_running = False
                return
            ...
            try:
                ...
                historical_data = await self.ib_client.request_historical_data(
                    symbol=request.symbol, sec_type=request.sec_type, exchange=request.exchange, currency=request.currency, duration=duration_str, bar_size=config["ib_size"], what_to_show="TRADES"
                )
                ...
                for bar_data in historical_data.get('bars', []):
                    try:
                        ...
                        normalized_bar = NormalizedBar(
                            venue=request.exchange, instrument_id=f"{request.symbol}.{request.exchange}", timeframe=timeframe, timestamp_ns=int(bar_time.timestamp() * 1_000_000_000), open_price=float(bar_data['open']), high_price=float(bar_data['high']), low_price=float(bar_data['low']), close_price=float(bar_data['close']), volume=float(bar_data['volume']), is_final=True
                        )
                ...
                progress.error_count += 1
                error_msg = str(e)
                self.logger.error(f"Error fetching data for {request.symbol} {timeframe}: {error_msg}")
                
                # Stop backfill if it's a connection error
                if "Not connected" in error_msg or "504" in error_msg:
                    self.logger.error(f"IB Gateway connection error detected - stopping backfill for {request.symbol}")
                    progress.status = "failed"
                    progress.last_error = f"IB Gateway connection error: {error_msg}"
                    self.is_running = False
                    return
                
                # For other errors, continue but add exponential backoff
                if progress.error_count >= 10:
                    self.logger.error(f"Too many errors ({progress.error_count}) - stopping backfill for {request.symbol} {timeframe}")
                    progress.status = "failed"
                    progress.last_error = f"Too many consecutive errors: {error_msg}"
                    return
                
                # Exponential backoff on errors
                backoff_time = min(30, 2 ** min(progress.error_count, 5))  # Cap at 30 seconds
    ...
    async def backfill_priority_instruments(self):
        ...
        for instrument in self.priority_instruments:
            request = BackfillRequest(
                symbol=instrument["symbol"], sec_type=instrument["sec_type"], exchange=instrument["exchange"], currency=instrument["currency"], timeframes=["1d", "1h", "15m", "5m"], # Start with most important timeframes
                priority=1
            )
    ...
    async def get_database_size_gb(self) -> float:
        ...
        try:
            ...
            size_query = """
                SELECT 
                    ROUND(
                        (pg_database_size(current_database()) / (1024.0 * 1024.0 * 1024.0))::numeric, 3
                    ) as size_gb
            """
    ...
    async def get_historical_data_stats(self) -> dict:
        ...
        try:
            ...
            stats_query = """
                SELECT 
                    COUNT(*) as total_bars, COUNT(DISTINCT instrument_id) as unique_instruments, COUNT(DISTINCT timeframe) as unique_timeframes, MIN(timestamp_ns) as earliest_data, MAX(timestamp_ns) as latest_data
                FROM market_bars
            """
            ...
            if result and len(result) > 0:
                stats = result[0]
                return {
                    "total_bars": stats.get('total_bars', 0), "unique_instruments": stats.get('unique_instruments', 0), "unique_timeframes": stats.get('unique_timeframes', 0), "earliest_data_ns": stats.get('earliest_data'), "latest_data_ns": stats.get('latest_data')
                }
            else:
                return {
                    "total_bars": 0, "unique_instruments": 0, "unique_timeframes": 0, "earliest_data_ns": None, "latest_data_ns": None
                }
    ...
    async def get_backfill_status(self) -> dict:
        ...
        return {
            "is_running": self.is_running, "queue_size": len(self.backfill_queue), "active_requests": len([p for p in self.progress_tracker.values() if p.status == "running"]), "completed_requests": len([p for p in self.progress_tracker.values() if p.status == "completed"]), "failed_requests": len([p for p in self.progress_tracker.values() if p.status == "failed"]), "database_size_gb": database_size_gb, "total_bars": data_stats["total_bars"], "unique_instruments": data_stats["unique_instruments"], "unique_timeframes": data_stats["unique_timeframes"], "earliest_data_ns": data_stats["earliest_data_ns"], "latest_data_ns": data_stats["latest_data_ns"], "progress_details": [
                {
                    "request_id": p.request_id, "symbol": p.symbol, "timeframe": p.timeframe, "status": p.status, "success_count": p.success_count, "error_count": p.error_count, "last_error": p.last_error
                }
    ...
    async def stop_backfill_process(self):
        ...
        if self.is_running:
            self.is_running = False
            self.logger.info("Backfill process stopped by user request")
        else:
            self.logger.info("Backfill process was not running")
    
    def force_stop_on_disconnect(self):
        """Force stop backfill process when IB Gateway disconnects"""
        if self.is_running:
            self.is_running = False
            self.logger.warning("Backfill process force stopped due to IB Gateway disconnect")
            
            # Mark all running progress entries as failed
